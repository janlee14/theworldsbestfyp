...
- summary 必须像一句给管理层看的判断，直接指出这家公司应如何借助 RWA.LTD 启动或推进非金融RWA代币化
- summary 必须简洁、有行动导向、有战略判断，控制在 1-2 句话
- report 使用 markdown 输出，避免过度密集的 bullet points，正文尽量多用自然段分析

你必须返回一个 JSON 对象，并且只能返回 JSON，不要输出任何额外解释。

JSON 格式必须是：
{{
  "summary": "一句强而有力的战略总结，直指这家公司应如何借助 RWA.LTD 启动或推进非金融RWA代币化。",
...
""".strip()
    ...
    try:
        ...
        if not summary:
            summary = (
                "该公司具备启动非金融RWA代币化的现实基础，建议优先从低风险、易验证、能直接体现业务价值的场景切入，并与 RWA.LTD 进一步讨论试点落地路径。"
                if is_zh
